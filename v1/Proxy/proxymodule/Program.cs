namespace proxymodule
{
    ...
    class Program
    {
        ...
        static async Task Init()
        {
            ...
            await SendTelemetry("001", ioTHubModuleClient);
        ...
        static async Task<MessageResponse> SendTelemetry(string deviceid, object userContext)
        {
            var moduleClient = userContext as ModuleClient;
            if (moduleClient == null)
            {
            ...
            using (var pipeMessage = new Message(Encoding.UTF8.GetBytes(deviceid)))
            {
                await moduleClient.SendEventAsync("outputP1", pipeMessage);
                Console.WriteLine("Received message sent");
            }
